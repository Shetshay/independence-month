...
public:
    ...
    float width;
    float height;
    Lz(float x, float y, float w, float h)
    {                    // Landing zone
...
void physics()
{
    ...
    if (lander.pos[1] <= lz.pos[1] + 10.0f && lander.pos[0] <= lz.pos[0] + 50.0f && lander.pos[0] >= lz.pos[0] - 50.0f && lander.pos[1] >= lz.pos[1] - 10.0f)
    {
        ...
        if (g.detach == false)
        {                                      // change variable name to snapon
    ...
    if (lander.pos[1] <= lz2.pos[1] + 10.0f && lander.pos[0] <= lz2.pos[0] + 50.0f && lander.pos[0] >= lz2.pos[0] - 50.0f && lander.pos[1] >= lz2.pos[1] - 10.0f)
    {
        g.shiptrackX[0] = lander.pos[0];
        // cout << lander.angle << endl;
        if (lander.angle >= -8 && lander.angle <= 8)
...
void render()
{
    ...
    glTranslatef(lz.pos[0] = g.nxtlanderX, lz.pos[1], 0.0f); // Lander spawning
    glBegin(GL_QUADS);
